...
export default function Button({
  ...
  const elementProps = href ? { href, ...props } : { type, onClick, ...props };

  return (
    <Tag
      {...elementProps}
      className={`group/button relative flex cursor-pointer items-center gap-2 rounded-lg border border-white/20 bg-black py-2 pr-4 pl-11 tracking-tight ${className}`}
    >
